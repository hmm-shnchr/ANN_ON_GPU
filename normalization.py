...
class Normalization:
    ...
    def run_predict(self, dataset):
        if self.norm_format == "Normalization":
            dataset_normed = self.__normalize(cp.deepcopy(dataset))
        if self.norm_format == "Standardization":
            dataset_normed = self.__standardize_predict(cp.deepcopy(dataset))
        if self.norm_format == "None":
            dataset_normed = self.__none(cp.deepcopy(dataset))
        return dataset_normed
    ...
    def inv_run_predict(self, dataset_normed):
        if self.norm_format == "Normalization":
            dataset_inv = self.__inv_normalize(cp.deepcopy(dataset_normed))
        if self.norm_format == "Standardization":
            dataset_inv = self.__inv_standardize_predict(cp.deepcopy(dataset_normed))
        if self.norm_format == "None":
            dataset_inv = self.__inv_none(cp.deepcopy(dataset_normed))
        return dataset_inv
