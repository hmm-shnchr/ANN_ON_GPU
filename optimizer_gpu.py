    ...
    def update(self, params, grads):
        if self.m is None and self.v is None:
            ...
            for key, val in params.items():
                ...
                self.v[key] = np.zeros_like(val)
        self.iters += 1
        lower_lr = self.final_lr * (1.0 - 1.0 / ((1.0-self.gamma) * self.iters + 1.0))
        higher_lr = self.final_lr * (1.0 + 1.0 / ((1.0-self.gamma) * self.iters))
        lr = self.lr * np.sqrt(1.0 - self.beta2**self.iters) / (1.0 - self.beta1**self.iters)
        for key in params.keys():
            self.m[key] += (1.0 - self.beta1) * (grads[key] - self.m[key])
            self.v[key] += (1.0 - self.beta2) * (grads[key]**2 - self.v[key])
            lr_t = np.clip(lr / (np.sqrt(self.v[key]) + self.eps), lower_lr, higher_lr)
